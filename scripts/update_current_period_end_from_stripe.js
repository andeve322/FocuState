#!/usr/bin/env node
/*
Admin script to backfill `current_period_end` (and `next_billing_date`) from
Stripe's upcoming invoice for all users who have a `stripeSubscriptionId`.

Features:
- Dry-run mode (default) prints what would change
- `--apply` actually writes updates to Firestore
- Pagination and batch processing to avoid loading entire collection at once
- Resilient to missing subscriptions/customers

Usage:
  # Dry run (safe)
  STRIPE_SECRET_KEY=sk_live_xxx GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
    node scripts/update_current_period_end_from_stripe.js --limit 200

  # Apply changes
  STRIPE_SECRET_KEY=sk_live_xxx GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
    node scripts/update_current_period_end_from_stripe.js --limit 200 --apply

Notes:
- Make sure the service account has Firestore write permissions.
- The script uses the Stripe Node SDK. Install with `npm install stripe firebase-admin`.
- Consider running against a small `--limit` first to validate behavior.
*/
...
try {
  ...
  // but also support a local serviceAccountKey.json in repo root if present.
...
async function processBatch(startAfterId) {
  ...
  // Process docs with limited concurrency
  let idx = 0;
  const results = [];
  async function worker() {
    while (idx < docs.length) {
      ...
      try {
        ...
        // Prefer upcoming invoice's next_payment_attempt (portal date)
        let nextAttempt = null;
        try {
          const upcoming = await stripe.invoices.retrieveUpcoming({ subscription: subscriptionId });
          if (upcoming && upcoming.next_payment_attempt) nextAttempt = upcoming.next_payment_attempt * 1000; // ms
        } catch (e) {
          // If upcoming invoice retrieval fails (resource_missing etc), we'll fall back
          console.warn(userId, 'retrieveUpcoming err:', e && e.message ? e.message : e);
        }

        // If no upcoming.next_payment_attempt, fall back to subscription.current_period_end
        if (!nextAttempt) {
          try {
        ...
        if (APPLY) {
          // Write top-level doc and mirror into preferences
  ...
  try {
    while (true) {
      ...
      for (const r of docs) {
        ...
        if (r.wouldApply) applied++; // count in dry-run as "would have been applied"
      ...
      // If fewer than limit docs were returned, we're at the end
      if (docs.length < LIMIT) break;

      // Wait a short interval to reduce burst load
